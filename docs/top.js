...
function charLink(name, charId, size = 64) {
    ...
    return `${img}<a href="./#${encodeURIComponent(name)}" class="char-link">${name}</a>`;
}
...
function orgsHtml(entry) {
    ...
    if (entry.corporation_id && entry.corporation_name) {
        orgs.push(`<a href="https://zkillboard.com/corporation/${entry.corporation_id}/" target="_blank" rel="noopener" class="org-link" title="${entry.corporation_name.replace(/"/g, '&quot;')}"><img src="${corpLogoUrl(entry.corporation_id)}" class="org-logo" alt="" loading="lazy"></a>`);
    }
    if (entry.alliance_id && entry.alliance_name) {
        orgs.push(`<a href="https://zkillboard.com/alliance/${entry.alliance_id}/" target="_blank" rel="noopener" class="org-link" title="${entry.alliance_name.replace(/"/g, '&quot;')}"><img src="${allianceLogoUrl(entry.alliance_id)}" class="org-logo" alt="" loading="lazy"></a>`);
    }
    ...
    top10.forEach((entry, i) => {
        ...
        row.innerHTML = `
            ...
            <div class="leader-info">
                ${charLink(entry.name, entry.id, 64)}
                ${orgsHtml(entry)}
            </div>
